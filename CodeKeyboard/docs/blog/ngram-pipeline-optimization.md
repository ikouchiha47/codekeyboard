# From Hang to 11 Minutes: Optimizing a 583MB N-gram Pipeline

*How we turned a pipeline that OOM'd, then hung for 2+ hours on AWS, into one that builds a 35.9M-row language model in ~11 minutes.*
...
**Top-k dominance pruning.** A word that only appears in the backoff distribution scores `λ·P_backoff(w|w2)` — a monotonic function. So only the top `max_followers` backoff entries can ever reach the top-`max_followers` result. Use `heapq.nlargest` instead of sorting everything.
