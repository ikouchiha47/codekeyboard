# Three smoothing variants for the keyboard n-gram model: KN vs Katz vs SwiftKey WDP
...
- **Katz solves: "make the discounting robust enough to survive aggressive
  ...
  `β` that keeps the seen + unseen distribution summing to 1. It's simpler
...
- **`bigram_ctx(ctx, total, distinct_n)` — per-context bookkeeping.** For each
  ...
  **λ(w1) = d · distinct_n / total** — "how much probability mass should this
...
backoff words scaled by the normalizing backoff weight β (so the distribution
still sums to 1):
...
*"Seen followers get the discounted MLE `d_r·c/total`. `seen_mass` is how much
...
distribution still sums to 1. Unseen words enter ranked by `β·p(w|w2)`, which
...
backoff; `ctx_joint·p` weights that gap by how likely the whole phrase is.
...
backoff words score exactly `β·p(w|w2)`, so their weighted difference is 0 and
