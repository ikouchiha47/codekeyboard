...
def score_trigram(followers: dict, backoff: dict, d_tri: dict, max_f: int,
                  delta: float, ctx_joint: float):
    """SwiftKey-style weighted-difference pruning trigram scoring.

    Keep follower w iff
        p(w1 w2 w) * |log p(w|w1 w2) - log(beta * p(w|w2))| > delta
    where p(w1 w2 w) = p(w|w1 w2) * p(w2|w1) * p(w1) is the chained joint
    probability (ctx_joint = p(w2|w1) * p(w1), constant per context).

    Unseen backoff words score exactly beta * p(w|w2), so their weighted
    difference is 0 and they are pruned — they replicate the bigram tier and
    add no trigram information. A context whose followers all fall below the
    ...
    for w, p in seen:
        ...
        if bo_p <= 0:
            diff = float("inf")  # no backoff estimate -> definitely informative
        else:
            diff = abs(math.log(p) - math.log(bo_p))
...
def step_score(args, counts_db: Path, norm_db: Path, output: Path) -> None:
    ...
    if args.order >= 3:
        ...
        bi_total_cache: dict[str, int] = {}

        def bigram_total(w1: str) -> int:
            hit = bi_total_cache.get(w1)
            if hit is None:
                hit = counts.execute(
                    "SELECT SUM(count) FROM bigrams WHERE ctx=?", (w1,)).fetchone()[0] or 0
                bi_total_cache[w1] = hit
            return hit

        def ctx_joint(w1: str, w2: str) -> float:
            """p(w2|w1) * p(w1) — the context part of the chained joint prob."""
            row = counts.execute(
                "SELECT count FROM bigrams WHERE ctx=? AND w=?", (w1, w2)).fetchone()
            if row is None:
                return 0.0
            tot = bigram_total(w1)
            if tot <= 0:
                return 0.0
            p_w2_w1 = d_bi.get(row[0], 1.0) * row[0] / tot
            return p_w2_w1 * uni_p.get(w1, 0.0)
...
def parse_args(argv=None):
    ...
    # SwiftKey outputs are swiftkey-prefixed by default so they never collide
    # with the KN (assets/kn/) or Katz (assets/kz/) files on S3 or when copied
    # back into the app assets.
    p.add_argument("--output", default="swiftkey_trigrams.json", help="trigrams.json path")
    p.add_argument("--output-bigrams", default="swiftkey_bigrams.json")
    p.add_argument("--output-bigrams-support", default="swiftkey_bigrams_support.json")
    ...
    p.add_argument("--delta", type=float, default=1e-6,
                   help="weighted-difference pruning threshold (SwiftKey WDP)")
