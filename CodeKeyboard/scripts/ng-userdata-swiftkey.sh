...
echo "started $(date -Is) swiftkey" | aws s3 cp - "s3://${BUCKET}/status/started.txt" --region "$REGION"
/usr/bin/python3 scripts/build_ngrams_swiftkey.py \
  --input input/swiftkey_all.txt \
  --output out/swiftkey_trigrams.json \
  --output-bigrams out/swiftkey_bigrams.json \
  --output-bigrams-support out/swiftkey_bigrams_support.json \
  --order 3 \
  --work-dir work \
  --workers 4 \
...
aws s3 cp out/build.log "s3://${BUCKET}/output/swiftkey_build.log" --region "$REGION"
